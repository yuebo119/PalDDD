...
| 裁决记录 | 2026-09-18 用户整体采纳六项：①③④ 正文修订（原生参数主路径 shape 2 / 补 ORDER BY / ADR-020 引用修正与结论强化）；4.1 表征测试本轮开做；4.2 接受 raw SQL；4.3 验收 = medium 复测基线 + 比值 ≤2× + <8ms 退出条款；4.4 不引入事务包裹（单语句原子性）；4.5 ChildSaga 开 ITM-787。遗留：ORM 优化旧清单「批量 lease 不做」为本裁决翻案，已在 open-items E 节记录。**2026-09-19 实施验收**：shape 2 已落地（637e73d，正确性回归全绿）；比值 4.22× 未达 ≤2×——按推荐改判「Lease 本体达标」（本体 0.25ms 达 Dapper 同量级 + 分配 -41%，比值锚定项为基准口径内的重灌、属 B1 范畴），基准比值记录为复合口径，B1 优先级提升建议已登记 open-items |
