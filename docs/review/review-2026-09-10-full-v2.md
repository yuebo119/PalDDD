...
> 评审基准：commit `69c06d8`（dev 分支）· **全仓重扫 + 验证轮**（逐 diff 验证 2026-09-10 v1 轮 ITM-615~647 共 64 项修复，diff 范围 d8746a6..69c06d8 = 88 文件 +2380/-167）
...
> 前序轮（2026-09-10 v1）报告：[`review-2026-09-10-full.md`](review-2026-09-10-full.md)（正文不可变，已含事后勘误段）

---

## 执行摘要

**评审结论**：机械防线与测试基线全绿（9 道防线 + Release 0W/0E + 14 项目 1098 用例），全仓重扫（src 213 文件 32067 行 + test 105 文件 28292 行 + 跨片 6 轴）未发现存量 P0；但**验证轮抓到 2026-09-10 v1 轮 64 项修复中 4 项自带缺陷（6.2%）**——其中 1 项 P1（CI 凭据门禁因 pipefail 缺位假绿，探针实锤）、3 项 P2（1 项为 2026-09-10 v1 轮修复引入的真回归）。另有 1 项 P2 为 2026-09-10 v2 轮新发现的跨栈精度疑点（待真库探针）。**验证轮协议（修复轮后必须跟验证轮）再次证明必要**——4 项缺陷全部是集成验证（编译+测试全绿）无法暴露的语义层问题。

| 指标 | 2026-09-10 v2 轮 | 2026-09-10 v1 轮 | 趋势 |
|------|:--:|:--:|:----:|
| P0 | 0 | 0 | → |
| P1 | 1 | 4 | ↓ |
| P2 | 4 | 26 | ↓ |
| P3 | 30（汇总） | 34 | → |
| 证伪数 | 9 | 24 | — |
| 2026-09-10 v1 轮修复自带缺陷 | **4/64（6.2%）** | — | 历史基线 3/50=6%（PD34 模型一致） |

### 验证轮总账（64 项 2026-09-10 v1 轮修复逐项核验）
...
| 2026-09-10 v1 轮发现 | v1 轮状态 | 2026-09-10 v2 轮验证 |
|----------|---------|---------|
...
```
2026-09-10 v2 轮应读：src 213 文件/32067 行（7 片）+ test 105 文件/28292 行（5 片）
...
2026-09-10 v1 轮对照：v1 轮 642 文件含 docs/scripts/config——2026-09-10 v2 轮 env 面由验证轮专项覆盖（v1 轮修复后的
...
三轴状态：
  ...
  实测轴 ⏸️：构建 0W/0E + 14 项目 1098 用例全绿；方言探针沿 2026-09-10 v1 轮结论（环境握手超时待 CI）；
            ITM-650 需真库探针（2026-09-10 v2 轮环境不可达）
```
...
机械防线    : 9 项全绿（见上）
2026-09-10 v1 轮修复 diff: d8746a6..69c06d8 = 88 文件 +2380/-167（7 commits）
```

### 1.3 范围声明

- **必须检查**：src+test 全量手写代码（真读重扫）；2026-09-10 v1 轮 88 文件修复 diff 的逐项语义验证；跨片 6 轴全局不变式。
- **明确不检查**：`*.g.cs`；`.ai/review/history/` 与 `docs/review/` 历史报告；第三方包内部实现；`docs/design/palorm-architecture.md`（2024 行，沿 2026-09-10 v1 轮 ❓ 标注）。
- **抽样策略**：docs/scripts/config 面未派独立地毯片（2026-09-10 v1 轮已全量读过且 2026-09-10 v2 轮无该面新增改动），由验证轮专项对**v1 轮改动集**做全量 diff+语义核验覆盖——存量部分沿 v1 轮报告结论；此为 2026-09-10 v2 轮范围收缩声明，相关结论标 ⚠。
...
| ID | 发现 | 证据 | 已对照模式 |
|----|------|------|:--:|
| ITM-649 | PalOrmSagaStateStore UPDATE 绑定原始 state.Error（2026-09-10 v1 轮 v65 引入回归，注释"两处共用收口"与代码不符） | Read :243/:244（`error = {state.Error}`）vs :221/:222（`{truncatedError}`）；姊妹 DapperSagaStateStore 两处均正确 | PD19+PD34 |
...
见清单 P3 段（2026-09-10 v1 轮连带 4 + src 10 + 测试防线 12 + 其余 4）。要点：
- **PD34 计数自激振荡×2**：2026-09-10 v1 轮刚修正的 ArchitectureBoundaryTests 方法数（41）与 ConfigureAwait 数（444）当轮再次失实（实际 37 / 447+）——修复轮自己的提交增删了计数对象。根因：**无机械锚的裸数字注定振荡**。建议 doc-consistency 补计数比对项或文档去裸数字化。
- **测试盲区三连**：HasValueSequence 多段分支（ITM-629 修复）零回归网、MySQL SslMode（v65 修复）零测试、EFCore 负 timeout 守卫零直接测试——2026-09-10 v1 轮修复的"传感器"缺口。
- **DiagnosticCoverageGate 形态①未收紧**：2026-09-10 v1 轮只收紧了形态②（diag 根），形态① `x.Id=="X"` 二元比较仍宽判定——收口不完整。
...
| OBS-4 | 2026-09-10 v1 轮弱断言棘轮（166）未反弹 | 新增测试全部行为断言，棘轮方向健康 |
...
| 原则 | 状态 | 证据 |
|------|:----:|------|
| 领域层零基础设施依赖 | ✅ | G2/G4/G5 PASS（2026-09-10 v1 轮修复未破坏） |
| 依赖方向外→内单向 | ✅ | 36 项目全图零循环（跨片轴 3 复核） |
| DIM 桥接替代反射 | ✅ | 2026-09-10 v1 轮 diff 反射模式扫描 0 命中（跨片轴 1） |
| AOT 三态分层 | ✅ | 显式 8/14 + 继承 14 复核成立；2026-09-10 v1 轮改动无新反射 |
| 聚合根保护不变量 | ✅ | 2026-09-10 v1 轮未触及 |
| 多实现契约一致性 | ⚠️ | PD17 两处新不对称（ITM-649 截断、ITM-652 SslMode）——均为 2026-09-10 v1 轮修复单边落地的连带 |
...
**收束判定**：**不可收束**。进入第三轮修复（范围小：1 P1 + 3 P2 易修项 + 1 待探针项）。按 PD37 轮次分级——2026-09-10 v2 轮发现量小且集中（2026-09-10 v1 轮修复连带），修复轮应**小改小评**（不放大为新一轮全量地毯），修复后跟一轮轻量验证（diff 验证 + ITM-648 探针复验 + 计数锚落地）即可收束。
...
```
2026-09-10 v2 轮局限：
- docs/scripts/config 存量面未独立地毯（由验证轮专项覆盖 2026-09-10 v1 轮改动集；见 1.3 范围声明）。
- ITM-650 未做真库探针（PG/MySQL 应用层握手超时沿 2026-09-10 v1 轮结论）——按证据分级标 ⚠，修复前必须补。
- 方言实测轴沿 2026-09-10 v1 轮 SKIP 待 CI。
- 测试 P3 批（清单 #30）为 13 条小项合并，未逐条独立探针。
- Messaging.Integration 4 个失败未重跑归类（沿 2026-09-10 v1 轮 BrokerUnreachable 结论，2026-09-10 v2 轮无 broker 侧改动）。
```

### 4.3 元审计自检

```
□ Z0 覆盖度: src+test 全量真读；验证轮专项覆盖全部 2026-09-10 v1 轮改动；范围收缩（env 面）已声明 ⚠   ✅
...
□ 一致性: 摘要 ↔ 清单 ↔ 收束判定一致                                                ✅
□ 对比: 与 2026-09-10 v1 轮 64 项逐项对账（59 通过/4 缺陷/2 振荡）                                ✅
违反项: 无
```
...
2. **2026-09-10 第三轮修复**：ITM-652（PG SslMode 镜像）+ ITM-651（fixture + PublishException 测试）+ 计数锚（PD34 根治）。
...
**修复轮完成**：ITM-648~652（1 P1 + 4 P2）**全部清偿** + P3 批 29/30；清单进度 97%（34/35，余 1 条 RollbackAsync 姊妹收口留 2026-09-11 v3 轮）。
...
**2026-09-10 v1 轮修复缺陷第 5 项（修复轮证伪发现）**：2026-09-10 v1 轮 ITM-637 的 `TryAddEnumerable(ServiceDescriptor.Singleton<IHostedService>(factory))` 写法在 MS.DI 下**首次调用即抛 ArgumentException**（factory 注册的 ImplementationType 取委托返回类型 = IHostedService 与 ServiceType 不可区分）——即该修复形态使 `AddPalPostgreSqlOutboxNotifier` 完全不可用（比验证轮清单预设的"第二次静默忽略"更严重）。已改双泛型 `Singleton<IHostedService, PostgreSqlOutboxNotifier>(factory)` 修复。**2026-09-10 v1 轮修复自带缺陷总数：5/64（7.8%）**——验证轮 + 修复轮两轮共抓出 5 项，全部为语义层问题。
