...
| 判定 | 条数 | 处置 |
|------|:----:|------|
| 真实缺陷 | 41 | 已修（ITM-726~749 + 764~765 + 769 + 774 + 763 + 775 + 778~780 + 783~784 + 788~793，含全部 4 条 critical） |
| **已声明的设计决定（误报）** | 22 | 驳回（ITM-750~761 + 766~768 + 770~773 + 776 + 781~782）——**记录在此防后续审计重复提出** |
| 已裁决·排队中 | 2 | ITM-762 与 ITM-777（同族：跨栈 fencing 契约）——**已有 ADR 覆盖**：ADR-020 决策 2 + 状态更新第 5 点，v3.0 窗口执行 |
| 需裁决 | 0 | ITM-763 已于 2026-09-16 按 ADR-023 方案 A 实施完毕（见下节标记 [x]） |
| 待查（已核实、未处理） | 2 | ITM-785（时钟推进顺序，blast radius 大，建议随 ITM-707 评估）· ITM-786（ActivityListener 跨项目隔离，低危） |
...
## 一、已修条目（ITM-726~749、764~765、769、774、775、778~780、783~784、788~793）
...
| 774 | G7 反射守卫漏检 `MakeGenericMethod`（清单只有 `MakeGenericType`，而生产代码 `Saga.cs:663` 在用、两份豁免文案亦以它命名）→ 未标注调用会静默通过 | 中/易 | `8a1d472` | 加 1 个 `isTarget` 分支 + 2 用例（R1b 无豁免必红 / R2b 豁免置零）· SourceCodeGuardTests 15/15 · 变异探针复现「0 处检出」· 提交用 `ALLOW_TEST_ONLY_CHANGE=1` 并写明豁免理由（守卫按设计实现于测试项目，改动方向是加强非修绿） |
| 775 | 被 ITM-069 拒绝的动态步骤污染执行轨迹（`RecordExecutedStep` 无条件追加键，调用点在校验之前）→ `SagaCompensation` 会补偿从未执行的步骤 | 中/中 | `afa1df2` | **先红后绿**实证（未修时轨迹断言按预期失败）· Transactions 全量 177/177 · 用例前置断言用**异常消息**而非异常类型（上一版弱断言导致非区分性，已纠正） |
| 778 | `FakeTimeProvider.CreateTimer` 对 `dueTime < 0`（含 `Timeout.InfiniteTimeSpan`）算出**已过期**时刻 → 零增量快进即触发（真实 Timer 语义为永不触发） | 高/易 | `abfd3b2` | +1 用例 · 变异探针（表达式+入列守卫两处都须还原才复现，实证"只改一处不足"）· Core 316/316 · Transactions 177/177 |
| 779 | `FakeTimeProvider.AdvanceNowAndTriggerTimers` 单回调抛出即中断批次 → 其余已出列计时器永不触发 + 假时钟冻结 | 高/中 | `abfd3b2` | +1 用例（断言三点：原异常按调用栈重抛 / 另一计时器仍触发 / 时钟已推进）· 变异探针转红 |
| 780 | `FakeTimeProvider._timers` 无同步（SUT 后台线程 CreateTimer × 测试线程快进）→ `InvalidOperationException`/集合损坏；`FakeTimer.Dispose` 不移除条目 → 长跑 worker 累积 | 中/中 | `abfd3b2` | 锁内快照+移除、回调**在锁外**触发（防重进入死锁）· Dispose 同步移除 · 无既有测试依赖原行为（Core+Transactions 全绿） |
| 783 | `AllocationContractTests.MeasureAllocation` 是全文件唯一裁判却从未被验证 → 若恒返回 0，所有「零分配」断言变恒真 | 高/易 | `009d859` | +1 正对照用例（对必然分配的动作要求非零且量级正确）· 变异 A（动作改不分配）→ 该用例失败 ✓ |
| 784 | `DomainEvents_Foreach_EnumeratorZeroAllocation`（核心契约）循环体为空 → JIT 可消除整段遍历，断言恒真 | 高/易 | `009d859` | 循环体计数经静态观测点写出 + 循环外断言读回 `IsEqualTo(100)`（遍历结果可观测、消除路径切断）· 变异 B（观测点不携带循环结果）→ 该用例失败 ✓ |

| —— | **以下为 2026-09-18 损伤审查 → 2026-09-19 清偿批**（编号接维护者已用的 ITM-787 之后）—— | —— | —— | —— |
| 788 | 回滚 M3-3 深拷贝（命中 v26 P3 声明设计：删声明而非回应声明，失败模式被静默翻转——并发写可抛→漏补偿无兜底；init→set 公共 API 破坏） | 高/中 | `523cce9` | set 访问器零依赖 grep 证实 · Transactions **194/194**（含 3c887c7 后全部新测试）· 两处 v26 声明注释复原 + 回滚勘正段；深隔离归 v3.0 窗口凭 ADR 重提 |
| 789 | 两处 IDE0005 未用 using（DapperDiRegistrationTests:10 / bench Program:2）致 check-all 持续红 | 低/易 | `8e97e9c` | check-all 三段全过 exit 0 · bench 项目此前未参与 format 分析故未暴露 |
| 790 | CI 无 format 门禁 → IDE0005 直达主干（3c887c7 实证；IDE0005 不在编译警告面） | 中/易 | `8e97e9c` | build-and-test 新增 Format verify 步骤（`--verify-no-changes`） |
| 791 | 公共 API 快照不记访问器 → init→set 破坏不可见（M3-3 实证：两属性翻转，快照零变化） | 中/中 | `fa052a7` | 生成器对 init-only 追加 ` [init]`（IsExternalInit modreq 判定）· 金标再生成 62 处标记 · **变异探针**（init→set + CA2227 抑制的完整形态）→ 精确红在 `:866` `[init]` 缺失 ✓ · 裸翻转另有 CA2227 编译期拦截（两层覆盖） |
| 792 | 租约索引只进建库 DDL，存量库无迁移说明 | 低/易 | `3336aae` | `docs/migration/lease-index-2026-09-17.md`：PG/SQLite 幂等语句、MySQL 8 无 IF NOT EXISTS 的两种替代、三方言验证；EF 栈走 EF 迁移 |
| 793 | 「审计条目命中声明注释」无流程规则（M3-3 链路：知道有声明→仍评低风险→删声明） | 中/易 | `3336aae` | AGENTS.md §3 陷阱表入册（现象/规避/机械层待办）；V11 管格式 + 本条管触发时机，两层闭环 |
...
## 二、驳回条目（ITM-750~761、766~768、770~773、776、781~782）— 各附判断依据

> **本节的用途**：这 12 条是模型的 critical/high 断言，读码后确认属**已声明的设计决定**。
> 记录依据是为了避免后续审计/下一个审查者重新提出——即本仓 AGENTS.md 的
...
| 781 | `FakeTimeProvider.CreateTimer` 静默丢弃 `period`（周期计时器只触发一次） | `TestHelpers.cs:215` 注释「P3 声明：period 有意忽略——FakeTimer 仅支持一次性到期语义（Saga/Outbox 测试场景不需要周期计时器）」。与 `Change` 抛异常的不对称是有意的（`Change` 被调用说明调用方依赖重设语义） |
| 782 | `FakeTimer.Change` 抛 `NotSupportedException`（而非返回 false） | `TestHelpers.cs:222-223` 及 `FakeTimeProviderTimerTests.Change_Always_Throws_NotSupported` 锁定：ITM-282（R44）把「恒 false」改为总是抛，避免调用方静默得到 false——属既有契约且已有用例防回退 |
...
## 四、需裁决 / 待查条目（ITM-763、775、785~786）

### [x] ITM-763 · `IUnitOfWork` 嵌套事务语义：三栈分歧（两栈静默提交外层事务）· 可信度 ✅
...
### [x] ITM-775 · `Saga.cs` 动态步骤执行轨迹的写入时机 · **已结项**

> **结论（2026-09-16 复核）**：模型断言**成立**，已修复（提交 `afa1df2`），条目升入第一节。

- **缺陷**：`RecordExecutedStep`（`Saga.cs:313-324`）**无条件**把 `stepKey` 追加进
  `result.ExecutedStepKeys`；动态分发路径的调用点在校验**之前**（`:824` vs `:834`），故被
  ITM-069 拒绝的 dispatch 已污染轨迹，而 `SagaCompensation` 契约是「只补偿实际已执行的步骤」。
- **修复**：调用点移到校验之后（仍在执行循环之前，正常路径时机不变）。

**⚠️ 误判纠正（方法学教训，值得留档）**：本条上一轮被记为「反证未过 → 撤回」。真因是
**我的变异探针有 bug**——脚本用 `findIndex` 定位 `SafeObserveStartedAsync` 锚点，取到的是
文件中**第一处**（属另一个方法），把记录调用插到了错误位置，等同从动态路径删掉该调用 →
键自然不在轨迹里，测试「修复前后均通过」被误读为因果链断裂。**探针的注入点本身必须校验**。

**新验证（顺序反转为先红后绿）**：① 未修状态跑新用例 → 前置断言（异常消息含「不支持事件
路由分发」）通过、轨迹断言失败 = 缺陷复现；② 应用修复 → 同用例通过。新用例相对上一版的
关键改进：**前置断言从异常类型改为异常消息**——`InvalidOperationException` 可由多条路径抛出，
仅断言类型会命中"别的路径"而让测试恒真（典型非区分性弱断言）。

### [ ] ITM-785 · `FakeTimeProvider` 时钟在回调**之后**推进 · 可信度 ✅（已核实，未处理）

- **维度**：测试基建语义 · **优先级**：P2 · 危害: 中 · 复杂度: 中
- **问题**：`AdvanceNowAndTriggerTimers` 先把回调跑完再赋值 `_now = threshold`，故回调内
  `GetUtcNow()` 观察到的是**旧时间**（真实 Timer 观察到的是触发时刻）；且末尾赋值是无条件覆盖，
  会**回滚**回调内嵌套的 `Advance`/`Set`，回调内新建的计时器也基于陈旧 `_now` 计算。
- **2026-09-16 ocr-scan 轮为何不改**：这是真实的语义差异，但改动会翻转所有依赖「回调观察到旧时间」这一行为的
  既有测试——**blast radius 大**。在测试基建上做未评估的语义变更，风险高于收益。
- **建议**：与 **ITM-707**（挂钟延迟测试改 FakeTimeProvider）一并评估——那次迁移会重写大批
  依赖时间的测试，正是把该语义一次性对齐的窗口。
- **涉及文件**：`test/PalDDD.Testing/TestHelpers.cs`（`:186` 附近）

### [ ] ITM-786 · ActivityListener 跨项目隔离依赖 `_createdAt` 比较 · 可信度 ⚠（低危）

- **维度**：测试基建语义 · **优先级**：P3 · 危害: 低 · 复杂度: 易
- **问题**：`_createdAt` 由字段初始化器捕获（早于 `AddActivityListener`），故该窗口内启动的
  activity（属上一轮测试）仍会被接纳；反向过滤又会丢弃「构造前启动、构造后结束」的长时 activity。
- **涉及文件**：`test/PalDDD.Testing/TestHelpers.cs`（`:35` 附近）
