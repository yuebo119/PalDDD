...
| D2 | 192.168.200.120 服务稳定性 | **状态不稳定，使用前先实测**（2026-09-14 四次变动：早间"全部实测通过" → 13 时半坏（TCP 通 AMQP 无响应 + Kafka 不可达，Messaging 5 失败，ITM-675）→ 约 14 时恢复（8/8）→ 21 时再现半坏（TCP 11ms 可达但 Kafka GetMetadata/Rabbit AMQP 握手均失败，8/8 快速跳过））；半坏特征 = TCP 开、协议无响应。**注**：全跳过时 MTP 退出码为 **8**（非失败码 2）——本地循环判定须区分"环境跳过"（exit 8 且失败=0）与"测试失败"（exit 2） |
