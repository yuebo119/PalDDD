...
| W2 | T-06 v3.0 承诺兑现 | **已按建议改判并完成**：13 处（实测 24 处）承诺改指 4.0 窗口，与接口异步化捆进同一 major | `ba2945a` |
| W5 | T-21 去墙钟 | **已按建议收口**：加注"10ms 是调度让出非轮询"；护栏调参**不做**（缺 CI 慢 runner 数据） | `04fd40b` |
| W3 | T-10 锁文件 | **受阻**（开锁文件即 restore 失败，需先隔离出哪个项目） | `783150d` |
| W3 | T-11 发布链 OIDC/签名 | **值得做 · 受阻于外部条件**：需维护者 NuGet.org 账号配置（Trusted Publishing 或证书） | — |
| W5 | T-18 PalORM 入覆盖率基线 | **值得做 · 受阻于外部条件**：需一次带 Docker 的 CI 跑（本地无 Docker 无 cobertura 产物） | — |
| W5 | T-20 性能回归门禁 | **值得做 · 受阻于测量前置**：需先在目标 runner 上测噪声地板，再定阈值（首期只能告警） | — |
| W4 | T-14 消 ChildSaga 反射 | **经论证不做**（见下） | — |
| W7 | T-25 安装器通用化 | **经论证暂缓**（见下） | — |

**进度：32 / 38 项完成（84%）+ 4 项受阻/暂缓 + 2 项经论证不做 · 主仓 48 个提交 + `.ai` 仓 7 个提交**

### 经论证不做的两项（理由留档，避免下轮重开）

**T-14 消 ChildSaga 反射 —— 不做**：反射路径已有 `[RequiresDynamicCode]` +
`[RequiresUnreferencedCode]` 声明，项目层 `IsAotCompatible=false`，**边界已诚实且机械可见**
（AOT 应用引用时构建期即得 `VerifyReferenceAotCompatibility` 诊断）。而动态派发与 AOT 本质
冲突——"工厂 API"实质要求编译期注册全部子 Saga 类型，等于改变特性性质而非消除反射。
除非出现真实的"AOT + ChildSaga"需求，这是打磨一条没人走的路。

**T-25 安装器通用化 —— 暂缓**：T-27 已修掉**失实**（`.ai/README.md` 写明"脚本在目标项目
无法直接运行"的分发边界）。剩余的通用化改造只在**存在第二个消费者**时才有价值；把 22 个
脚本改成通用的是为想象中的用户付成本。**触发条件**：出现第二个安装 `.ai` 的项目时重启。

### 新增待办（本轮发现，非原计划）

**T-39 扩 T-38 判定器的形态面**：serena dry-run 一次列出 24 处 `v3.0`，而 T-38 只抓到 13 处——
漏掉 11 处，形态为：关键词表不全（裁决/收敛/兑现需/契约面/接口扩展）、**顺序固定**（要求
"版本在前"，故"破坏性变更**留** v3.0"漏）、**只扫 `.cs`**（漏 `src/PalDDD.Prompts/.pal/prompts/*.prompt.md`）。
与 T-26 修 V25 的那类**同型**——即我修的判定器自身也有形态盲区。按 T-34 规程，扩面须同步
声明覆盖形态并补探针。
