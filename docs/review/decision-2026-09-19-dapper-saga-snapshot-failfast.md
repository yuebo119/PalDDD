...
**告警噪音权衡（R54-P2-2 补，2026-09-19 第五十四轮片1）**：配置缺失是**永久错误**（不会自愈）——fail-fast 意味着每 tick × 每条活跃 Saga 产生一条 Error 日志（30s 轮询 × N 条 Saga），无熔断/去重。这是 fail-fast 换来的可接受代价（静默丢数据 vs 可见噪音，取后者），但运维侧应以首条 Error 为行动信号（注册 `AddPalDapperSagaSnapshot` 后噪音即止），不应期望日志系统降噪。若未来 N 量大到成为问题，节流方案（每 SagaId 只报一次）为后续独立变更，本决策不预置。
