...
**A2 [高] Dynamic 车道补偿缺陷：路由目标失败时补偿一个从未执行的步骤**
(a) `ExecuteDynamicStepAsync` 在 ITM-069 校验之后、执行之前调用 `RecordExecutedStep(current, current, stepKey, startedAt)` 把 Dynamic 注册键写入 `ExecutedStepKeys`；真实执行体的 `matchedKey` 只在 attempt 委托成功后写入 `result`，attempt 抛异常时 `result` 被丢弃。而补偿侧按 `ExecutedStepKeys` 遍历并调用 `_stepsByKey[key].CompensateAsync`。
(b) `src/PalDDD.Transactions/Saga/Saga.cs:827`（提前记录）+ `:842`（matchedKey 仅在成功路径）；补偿查找 `src/PalDDD.Transactions/Saga/SagaCompensation.cs:59-61,87`；ITM-775 只把记录点从"校验前"移到"校验后"（`:822-826` 注释自述），未触及失败路径。
(c) `DynamicStep` 接受可选 `compensate` 委托（`DynamicStep.cs:31,35`）。路由目标重试耗尽失败时，补偿会调用这个只做了路由、没有任何副作用的步骤的补偿动作；同时 `matchedKey` 不在 `ExecutedStepKeys` 中，真实失败步骤只靠 `failedStep` 参数兜底。测试 `DynamicStepRouteRejectionTests` 只覆盖拒绝路径，制造了"已修复"的假象。
(d) 高　(e) `[事实]`（代码路径与记录时机可复现，补偿键查找已核实）；`[判断]`（误补偿的业务后果，补偿动作通常是外部副作用回滚）。
...
| M0-3 | Dynamic 失败路径表征测试（先红）：构造 DynamicStep 带 compensate 委托 + 路由目标 attempt 恒失败，断言失败后补偿**未**调用 Dynamic 的 compensate（当前会调，测试红） | `test/PalDDD.Transactions.Tests/`（新增） | 新测试在当前代码上失败（证明缺陷存在），修复后转绿 | S | 低 | 无 |

**里程碑 1 · 关键修复（正确性与安全）**

| ID | 标题与描述 | 文件/区域 | 验收标准 | 努力 | 风险 | 依赖 |
|---|---|---|---|---|---|---|
| M1-1 | 修 Dynamic 补偿缺陷：把 `RecordExecutedStep(stepKey)` 从执行前移到 attempt 成功之后（与 Normal/FanOut 车道一致），或改为失败路径只记 matchedKey。需同步更新 ITM-775 注释与双键轨迹表征测试 | `src/PalDDD.Transactions/Saga/Saga.cs:827,842` | M0-3 测试转绿；`Success_RecordsBothKeys` 仍绿；`SagaLaneCharacterizationTests` 全绿 | M | 中（触及四车道共享的轨迹不变量，须回归全部 Saga 表征测试） | M0-3 |
