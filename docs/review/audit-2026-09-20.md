...
(a) `gate-audit.cs` 矩阵输出 `ci-coverage` 行 WIRED=yes、SELFTEST=yes、**PROBED=`-`**；17 个接线门禁里只有 5 个（secret-scan/encoding-gate/dapper-param-guard/gate-lite/verify-conventions）有隔离式变异探针。脚本 `scripts/ci-coverage.cs:191-192`（阈值 NaN/Infinity）与 `:214-215`（line-rate NaN）两处注释明确记录"全仓扫描修复（fail-open）"，即这个门禁真实发生过恒假比较导致静默放行。
(b) 上述位置。
(c) 按项目自订规程（新增门禁须带 `--selftest` 且变异验证能红），coverage 门禁的 selftest 是构造最小 XML 的单元式自证，没有"注入坏输入→断言非零退出"的隔离探针；阈值判定逻辑若再被改坏，CI 只会看到绿灯。
(d) 中　(e) `[事实]`。

> **实施结论（M0-2，已落地）**：`ci-coverage` 已接入隔离式变异探针（矩阵 PROBED 列转 yes，探针 12/12 通过）。为使探针能直达被测逻辑，`ci-coverage.cs` 新增 `--enforce-only` 入口（跳过 build/test/merge 前四步，仅跑第 5/6 步门禁判定）——此前探针在空仓必然死于前四步，等于没探。三条探针分别覆盖：低于阈值必须 fail-closed（比较方向）、line-rate 非数字必须 fail-closed（NaN 回归）、合并报告缺失必须 fail-closed。**反向变异已验证**：临时把阈值比较改为恒假后，对应探针立即报 FAIL，证明探针本身能红而非恒绿。
