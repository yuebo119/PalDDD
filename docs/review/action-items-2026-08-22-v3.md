...
### [x] ITM-268 · F1：AotSample 终验恒真——租约遮蔽使 GetPending==0 在 bug 复活时同样成立
...
### [x] ITM-269 · F2：PalOrmSample 演示路径违反接口契约（GetPending 观测用途被当处理管线）
...
### [x] ITM-270 · PalOrmProjectionCheckpointStore lease_until 无 IsDBNull 容错（三十八轮只修了 Dapper 姊妹）
...
- **ITM-268**：终验改 `pending[0].Status == Processed` 状态断言 + Check 先于索引（防 IndexOutOfRange 绕过 failures 机制）+ 注释准确化。**mutation 自证闭环**：修复绿 → 临时改回原始引用 → 终验 FAILED + 样本 ❌ 退出 → 恢复绿（R40 恒真形态被根治的直接证据）。
- **ITM-269**：PalOrmSample 步骤 2-3 改走 Lease 路径（样本运行实证 `Outbox.Lease: Count=1, Owner=palorm-sample` + `MarkProcessed: Status=Processed`）+ 终态断言；`IPalOutboxStore.MarkProcessed` XML doc 补前置条件声明（未租约直标在 PalORM 放行/InMemory·Dapper 拒绝的语义相反面）。
- **ITM-270**：`IsDBNull(5) ? default : GetUtc(reader, 5)` 对齐 Dapper 姊妹（跨栈共用表场景 Dapper 正式 DDL 的 lease_until 可空）；传感器用宽松 DDL 直插 NULL 行（raw 参数绑 DBNull），新测试绿。
- **ITM-271**：五处勘正（OutboxSqlite 两处"不可翻译"旧措辞、SagaStateStoreTests 两处 AsyncLocal 残留、OutboxMultiDialect 类头 Base64、SagaStateDbContext Entry 语义、DapperInboxStore 抛点层）。
- **终基线**：build 0/0；测试 1044 = 999 通过 + 45 fail-closed + 0 代码失败（+1 传感器）；两样本 all passed；dialect-probe 40/40；机械全绿（棘轮 152；gate 21/22 之 G22=待提交）。
