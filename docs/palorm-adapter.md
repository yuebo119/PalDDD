...
- **动态表名**：不支持（表名编译期固化）
- **raw command 事务挂接（ITM-243 后的残余限制）**：Saga/Checkpoint/Idempotency 三 Store 的手动 reader 命令经 `PalOrmAmbientTransaction`（Session 键控 ConditionalWeakTable 弱引用表——R40 修复轮由 AsyncLocal 改造，异步方法内赋值不外流探针实证）挂接 **IUnitOfWork 开启的**活动事务——MySQL 活动事务下未挂 `cmd.Transaction` 的命令会抛 InvalidOperationException（MySqlConnector 严格校验）。直接调 `session.BeginTransactionAsync` 绕过 IUnitOfWork 时 raw command 仍不挂接（PalORM 5.3 无公开活动事务访问器，待上游提供后迁移）
