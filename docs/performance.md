...
| 全测试项目测试 | `for p in $(find test -name '*.Tests.csproj' ! -path '*/obj/*' ! -path '*/bin/*' \| sort); do dotnet test "$p" --no-restore; done`（MTP 禁用 slnx 批量——握手 → exit 5） | 通过；869 passed / 2 failed / 6 skipped（历史快照：2026-06-28；2026-09-23 实测：16 项目 1490 用例 = 本机 1430 通过 + 60 项无 Docker 跳过（PalORM 多方言 46 + Integration 14）由 CI Testcontainers 执行） |
