...
public static class PostgreSqlPalOrmExtensions
{
        ...
        int prewarmConnectionCount = 0)
    {
        ...
        {
            var session = DataSession<PostgreSqlProvider>.CreateAsync(resolvedOptions, default).GetAwaiter().GetResult();
        ...
        if (prewarmConnectionCount > 0)
        {
            services.AddSingleton<IHostedService>(sp =>
                new PalOrmPreWarmHostedService<PostgreSqlProvider>(
                    resolvedOptions,
                    prewarmConnectionCount,
                    sp.GetService<IPalLogger<PalOrmPreWarmHostedService<PostgreSqlProvider>>>()));
        }
