...
//   ｜ 为什么不用 List&lt;T&gt;？因为 List 无参构造仍分配 List 对象自身（v70 勘正：内部数组用 Array.Empty 单例不分配），每个 Add 可能触发扩容。
//   ｜ 单链表在无事件时只占 16B（两个 null 指针），有事件时每事件一个节点对象（无容器开销；v62 勘正：原"88B/事件"具体数字无法从当前字段布局推导，改定性描述）。
// ═══════════════════════════════════════════════════════════════
