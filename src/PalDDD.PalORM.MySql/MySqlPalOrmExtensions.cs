...
public static class MySqlPalOrmExtensions
{
    ...
    /// <see cref="DataSession{TProvider}.PreWarmAsync"/>（远程建连实测 ~8.5 ms/条，上游 XML）；
    /// 失败降级启动（记 Warning，不炸 host）。生产场景与 <c>options.MinPoolSize</c> 配合
    /// （只预热不设下限会被空闲修剪清空，上游 XML 明示）。
    /// </param>
        ...
        int prewarmConnectionCount = 0)
    {
        ...
        {
            var session = DataSession<MySqlProvider>.CreateAsync(resolvedOptions, default).GetAwaiter().GetResult();
        ...
        // hosted 包装（Singleton 工厂内从容器取可选日志面，缺省由服务内 NullPalLogger 兜底）。
        if (prewarmConnectionCount > 0)
        {
            services.AddSingleton<IHostedService>(sp =>
                new PalOrmPreWarmHostedService<MySqlProvider>(
                    resolvedOptions,
                    prewarmConnectionCount,
                    sp.GetService<IPalLogger<PalOrmPreWarmHostedService<MySqlProvider>>>()));
        }
    ...
    private static void DisposeQuietly(DataSession<MySqlProvider> session)
    {
