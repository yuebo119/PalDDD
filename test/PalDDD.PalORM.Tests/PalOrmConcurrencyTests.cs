...
/// 指 PalORM DataSession 库内部门禁，非本仓 CWT 事务挂接）。
...
public class PalOrmConcurrencyTests
{
    ...
    public async Task Outbox_Lease_Concurrent_NoDoubleDispatch()
    {
        ...
        if (File.Exists(dbPath)) try { File.Delete(dbPath); } catch { /* SQLite 连接池延迟释放属预期，残留文件 Guid 命名不冲突 */ }

        // TST-117：断言失败也清理 DB 文件（对齐下方 fencing 测试 try/finally 形态）
        try
        {
            // 主 session 建表 + 插入数据
