...
public class PalOrmProjectionMultiDialectTests
{
    ...
    public async Task Projection_Sqlite_TryStart_CreatesCheckpoint()
        => await Test_TryStart_CreatesCheckpoint(await new MultiDialectFixture().CreateSqliteAsync());

    [Test]
    public async Task Projection_PostgreSql_TryStart_CreatesCheckpoint()
        => await Test_TryStart_CreatesCheckpoint(await new MultiDialectFixture().CreatePostgreSqlAsync());

    [Test]
    public async Task Projection_MySql_TryStart_CreatesCheckpoint()
        => await Test_TryStart_CreatesCheckpoint(await new MultiDialectFixture().CreateMySqlAsync());

    private static async Task Test_TryStart_CreatesCheckpoint<TProvider>(DataSession<TProvider> session)
        where TProvider : IDbProvider
    {
        var store = new PalOrmProjectionCheckpointStore<TProvider>(session);
    ...
    public async Task Projection_Sqlite_MarkCompleted_PreventsReprocessing()
        => await Test_MarkCompleted_PreventsReprocessing(await new MultiDialectFixture().CreateSqliteAsync());

    [Test]
    public async Task Projection_PostgreSql_MarkCompleted_PreventsReprocessing()
        => await Test_MarkCompleted_PreventsReprocessing(await new MultiDialectFixture().CreatePostgreSqlAsync());

    [Test]
    public async Task Projection_MySql_MarkCompleted_PreventsReprocessing()
        => await Test_MarkCompleted_PreventsReprocessing(await new MultiDialectFixture().CreateMySqlAsync());

    private static async Task Test_MarkCompleted_PreventsReprocessing<TProvider>(DataSession<TProvider> session)
        where TProvider : IDbProvider
    {
        var store = new PalOrmProjectionCheckpointStore<TProvider>(session);
        var now = DateTimeOffset.UtcNow;
    ...
    public async Task Projection_Sqlite_Reset_RemovesSource()
        => await Test_Reset_RemovesSource(await new MultiDialectFixture().CreateSqliteAsync());

    [Test]
    public async Task Projection_PostgreSql_Reset_RemovesSource()
        => await Test_Reset_RemovesSource(await new MultiDialectFixture().CreatePostgreSqlAsync());

    [Test]
    public async Task Projection_MySql_Reset_RemovesSource()
        => await Test_Reset_RemovesSource(await new MultiDialectFixture().CreateMySqlAsync());

    private static async Task Test_Reset_RemovesSource<TProvider>(DataSession<TProvider> session)
        where TProvider : IDbProvider
    {
        var store = new PalOrmProjectionCheckpointStore<TProvider>(session);
        var now = DateTimeOffset.UtcNow;
