...
public static class PalOrmStoreFixture
{
    ...
    public static async Task<DataSession<SqliteProvider>> CreateAsync(CancellationToken ct = default)
    {
        ...
        await session.ExecuteAsync($"CREATE TABLE Events (GlobalPosition INTEGER PRIMARY KEY AUTOINCREMENT, EventId TEXT NOT NULL, EventName TEXT NOT NULL, StreamName TEXT NOT NULL, StreamVersion INTEGER NOT NULL, SchemaVersion INTEGER NOT NULL DEFAULT 1, ContentType TEXT NOT NULL DEFAULT 'application/json', Payload TEXT NOT NULL, Metadata TEXT, RecordedAt TEXT NOT NULL, ActorId TEXT, Reason TEXT)", ct);
        await session.ExecuteAsync($"CREATE UNIQUE INDEX idx_events_stream ON Events(StreamName, StreamVersion)", ct);
